...
int inode_indexlookup(struct unixfilesystem *fs, struct inode *inp, int blockNum) {
    ...
    // 🔧 Caso especial: algunos archivos (por ejemplo directorios) pueden tener ILARG
    // pero aún así usar bloques directos en i_addr[0] a i_addr[7]
    if ((inp->i_mode & ILARG) && blockNum < 8 && inp->i_addr[blockNum] != 0) {
